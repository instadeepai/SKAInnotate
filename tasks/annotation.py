...
class AnnotationRepository:
  ...
  def add_annotation(self, label: str, example_id: str, annotator_id: int) -> Annotation:
    ...
    if existing_annotation:
      ...
      logger.info(f"Update- example_id: {example_id} label: {label}")
    else:
  ...
  def get_annotations_for_example(self, example_id: str) -> List[Annotation]:
    # Implementation details for retrieving all annotations for a specific example
    return self.session.query(Annotation).filter_by(example_id=example_id).all()
  ...
  def get_tasks_assigned_to_annotator(self, annotator_id: int):
    annotator = self.session.query(Annotator).filter(Annotator.annotator_id==annotator_id).first()
...
class AnnotationWorkflow:
  ...
  def get_annotations_for_example(self, example_id: str) -> List[Annotation]:
    ...
    return self.annotation_repo.get_annotations_for_example(example_id)
  ...
  def get_tasks_assigned_to_annotator(self, annotator_id: int):
    return self.annotation_repo.get_tasks_assigned_to_annotator(annotator_id)
